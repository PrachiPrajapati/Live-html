...
(function( $ ) {
	...
	Slider.prototype = {
		...
		layout: function(){
			this.positionPercentages;

			if(this.reversed) {
				this.positionPercentages = [ 100 - this.percentage[0], this.percentage[1] ];
			} else {
				this.positionPercentages = [ this.percentage[0], this.percentage[1] ];
			}

			this.handle1Stype[this.stylePos] = this.positionPercentages[0]+'%';
			this.handle2Stype[this.stylePos] = this.positionPercentages[1]+'%';

			if (this.orientation === 'vertical') {
				this.selectionElStyle.top = Math.min(this.positionPercentages[0], this.positionPercentages[1]) +'%';
				this.selectionElStyle.height = Math.abs(this.positionPercentages[0] - this.positionPercentages[1]) +'%';
			} else {
				this.selectionElStyle.left = Math.min(this.positionPercentages[0], this.positionPercentages[1]) +'%';
				this.selectionElStyle.width = Math.abs(this.positionPercentages[0] - this.positionPercentages[1]) +'%';
		...
		calculateValue: function() {
			this.calculatedVal = null;
			if (this.range) {
				this.calculatedVal = [this.min,this.max];
                if (this.percentage[0] !== 0){
                    this.calculatedVal[0] = (Math.max(this.min, this.min + Math.round((this.diff * this.percentage[0]/100)/this.step)*this.step));
                    this.calculatedVal[0] = this.applyPrecision(this.calculatedVal[0]);
                }
                if (this.percentage[1] !== 100){
                    this.calculatedVal[1] = (Math.min(this.max, this.min + Math.round((this.diff * this.percentage[1]/100)/this.step)*this.step));
                    this.calculatedVal[1] = this.applyPrecision(this.calculatedVal[1]);
                }
				this.value = this.calculatedVal;
			} else {
				this.calculatedVal = (this.min + Math.round((this.diff * this.percentage[0]/100)/this.step)*this.step);
				if (this.calculatedVal < this.min) {
					this.calculatedVal = this.min;
				}
				else if (this.calculatedVal > this.max) {
					this.calculatedVal = this.max;
				}
				this.calculatedVal = parseFloat(this.calculatedVal);
				this.calculatedVal = this.applyPrecision(this.calculatedVal);
				this.value = [this.calculatedVal, this.value[1]];
			}
