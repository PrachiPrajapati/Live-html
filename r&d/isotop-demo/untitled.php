...
?>
	...
	<script src="<?php echo get_stylesheet_directory_uri(); ?>/js/icomon.js"></script>
	<script src="<?php echo get_stylesheet_directory_uri(); ?>/js/filter.min.js"></script>
...
<script>
	jQuery(document).ready(function(){
		jQuery('.faq .vc_toggle_title').on('click', function(){
			var thisParent = jQuery(this).parent('faq');
			if(thisParent.hasClass('vc_toggle_active')) {
				jQuery(this).parent().removeClass('vc_toggle_active');
				jQuery(this).siblings('.vc_toggle_content').hide();
			} 
			else {
				jQuery(this).closest('.faq-section').find('.faq').removeClass('vc_toggle_active');
				jQuery(this).closest('.faq-section').find('.faq .vc_toggle_content').hide();
